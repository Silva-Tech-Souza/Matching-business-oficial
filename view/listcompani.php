...
<body class="funcolinhas">
  ...
  <div class="main-wrapper" style="overflow: hidden !important;">
    ...
    <?php
            ...
            if($text != null && $text != "" && $text != "mysp"){
                $operacao = new Operations($dbh);
                $operacao->setNmOperation($text);
                $operacaoresults = $operacao->consulta("WHERE NmOperation = :NmOperation");
                if ($operacaoresults != null) {
                    foreach ($operacaoresults as $rowoperacao) {
                        $operation = $rowoperacao->idOperation;          
                    }
                }
            }
          ...
          <?php if($text == "mysp"){?>
            <?php 
                ...
                if ($resultSearch != null) {
                    foreach ($resultSearch as $rowSearch) {
                        $textoTopo = $rowSearch->Nome;
                        //get operation
                        $operacao = new Operations($dbh);
                        $operacao->setidOperation($rowSearch->coreBussinessID);
                        $operacaoresults = $operacao->consulta("WHERE idOperation = :idOperation");
                        if ($operacaoresults != null) {
                            foreach ($operacaoresults as $rowoperacao) {
                        ...
                        //searchBusiness
                        $searchbusiness = new SearchBusiness($dbh);
                        $searchbusiness->setidSearch($rowSearch->idSearch);
                        $searchbusinessresults = $searchbusiness->consulta("WHERE idSearch = :idSearch");
                        if ($searchbusinessresults != null) {
                            foreach ($searchbusinessresults as $rowsearchbusiness) {
                           
                                $idcoreb = $rowsearchbusiness->idBusiness;
                        ...
                        //subbussiness
                        $business = new Business($dbh);
                        $business->setidBusiness($idcoreb);
                        $businessresults = $business->consulta("WHERE idBusiness = :idBusiness");
                        if ($businessresults != null) {
                            foreach ($businessresults as $rowbusiness) {
                           
                                $textoTopo = $textoTopo . " - " . $rowbusiness->NmBusiness;
                            
                            }
                        }

                        //country
                        $countrysearch = new SearchCountry($dbh);
                        $countrysearch->setidSearch($rowSearch->idSearch);
                        $countrysearchresults = $countrysearch->consulta("WHERE idSearch = :idSearch");
                        if ($countrysearchresults != null) {
                            ...
                            $textoTopo = $textoTopo . " - $qtdcountry Countries";
            ...
            <div class="row " >
              ...
              <div class="col-12" style="padding: 0 !important; margin: 0 !important;">
                ...
                $SearchResults = new SearchResults($dbh);
                $SearchResults->setidSearch($rowSearch->idSearch);
                $resultSearchResults = $SearchResults->consulta("WHERE idSearch = :idSearch");
                if ($resultSearchResults != null) {
                    foreach ($resultSearchResults as $rowSearchResults) {
                  ...
                  $Operationselect->setidClient($rowSearchResults->idClientResultado);
                  
                  $resultsOperationselect = $Operationselect->consulta("WHERE idClient = :idClient");
                    ...
                    foreach ($resultsOperationselect as $rowOperationselect) {
                      ...
                      <div class="card card-body p-0 shadow rounded horizontal-scrollable" style="max-width: 400px; margin-left: 20px; display: inline-block; margin-right: 10px; vertical-align: top;">
                        ...
                        <div class="col-5 d-flex justify-content-start p-0 m-0 " style="height: 0px;">
                          ...
                          <h4 class="fonte-principal color-preto">&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa-solid fa-briefcase "></i>&nbsp;&nbsp;<?php echo  $NmBusinessCategory; ?></h4>
                          <?php }?>
                          ...
                          <div class="color-preto estilo-desc" data-card-id="<?php echo $rowOperationselect->idClient; ?>" style="overflow-wrap: break-word; max-width: 380px; word-wrap: break-word; font-size: 12px; text-align: justify;">
